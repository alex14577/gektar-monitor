...
- [[decisions/ADR-050-status-indicator-supersedes-countdown|ADR-050]] — Pulse-dot replaces countdown: `SseCycleStarted` event + `.check-status[data-state]` toggle; `next_fire_at`/`next_cycle_mmss` removed from `SseStatus`; supersedes ADR-048 (bd hiq3). **Amended 2026-06-05 (zb3):** pulse-dot удалён (lw5s); канонический механизм — `SseStatus(state="checking")` + server-push HTML `#header-status` (`event: status`); `evict_normal_replay` исключает transient из replay-слота; `_terminal_status_published` fallback-guard; OOB-ресинк `#header-status` в `GET /feed/count`
...
- [[decisions/ADR-060-backfill-sse-insertion-and-true-total-counter|ADR-060]] — Backfill SSE insertion at bottom + counter as true total: `SseLotNew.is_backfill` flag; JS reposition synchronous in htmx:sseMessage; `LotQueryService.count()` for true total; OOB counter removed from load-more (dr21, ddpf, hke7). Amended 2026-06-01 (gektar-monitor-0v9): counter de-duplicated to single `#feed-lot-count`; `.zone__title-count` removed. Amended 2026-06-03 (gektar-monitor-6jg): counter split into `div.feed-scope` «Показано N из M» (N=DOM card count `js-shown-count`, M=`count(filters)` `#feed-lot-count`) + «Всего в реестре: X» (X=`count_active()` global `#registry-count`, live via `/backfill/status` `active_lot_count`); 3-state cold-start indicator in `feed.js` (routine silence / «Заполняем реестр…» notice / `toast('Каталог обновлён')` once); `#backfill-progress` banner removed; `data-count="0"` hide rule removed (visible at 0); `GET /feed/count` returns M + OOB X (`_feed_count_resync.html.jinja`). Relies on ADR-066 (SSE membership) so DOM «shown» count is trustworthy. Amended 2026-06-04 (gektar-monitor-azc): page-guard на §z15 `htmx:sseOpen` re-sync — `htmx.ajax('/feed/count')` только если `#feed-lot-count` есть в DOM; без guard htmx null-target fallback свапал фрагмент в `document.body`, затирая `/settings` до «N лотов». **Amended 2026-06-05 (gyn):** insertion target переведён с `#feed` на `section#feed-zone-list` (afterbegin); JS синхронно перемещает карточку в позицию сортировки `(data-date-create DESC, data-lot-id DESC)` — чинит прод-баг: карточка вставлялась вне `section.zone`, заголовок «Список» терялся. `lot.status` вынесен на `span#lot-status-listener` (`hx-swap="none"`); инкремент pill «N новых» через MutationObserver с дедупликацией по батчу
...
- [[decisions/ADR-062-region-subscription-namespace|ADR-062]] — `region_subscriptions.region_id`: macro-id → subject site-id (совпадает с `lots.region_id` post-v8). Write-path разворачивает macro→subjects через `subjects_for_macros`; migration v9→v10 (MIN subscribed_at для пересечений 87/96); `count_active(region_ids: tuple)` IN-список. Чинит мёртвый cutoff-JOIN, suppression, delta-trigger (gektar-monitor-v1t)
- [[decisions/ADR-063-session-expired-iterate-propagation|ADR-063]] — `PaginatedListFetcher.iterate()` пробрасывает `SessionExpiredError` (был молчаливый `return` через generic except) → Backfill/FullScan публикуют `SseSessionExpired` и прекращают обход; реаутентификация остаётся ручной (UI/email). Чинит тихий недобор каталога при протухании сессии в середине обхода (gektar-monitor-sv3)
- [[decisions/ADR-064-region-param-noop-global-delta-count|ADR-064]] — донорский `region=` — no-op (перекрыт операторским `use_filter_pocket=1`; live-проверка region=1/region=2/bare → все `Найдено записей: 346`). Delta-trigger считает **глобальный** `count_active()` вместо подмножества `subjects_for_macros([region])` (чинит бесконечный backfill — delta не сходился). Fetch-петля схлопнута: monitor_cycle + full_scan делают **один** запрос/цикл (`regions[0]`), backfill не тронут; `settings.regions` остаётся для подписок/notify/view. Амендирует ADR-035 §I1, ADR-036 §H4, часть ADR-062 (gektar-monitor-rzj)
- [[decisions/ADR-065-feed-visibility-subject-membership|ADR-065]] — лента `/feed` показывает лоты по **членству-в-подписке** (есть строка в `region_subscriptions`), а НЕ по date-cutoff: все активные лоты подписанных субъектов независимо от `date_create`, неподписанные скрыты, `region_id IS NULL` показан. Date-cutoff (ADR-039 `passes_subscription_cutoff`) остаётся **только для email**. Новый флаг `LotFilters.filter_subscribed_subjects` (подзапрос `region_id IN (SELECT … FROM region_subscriptions)`, без JOIN; взаимоисключающ с `apply_subscription_cutoff`). Чинит fail-open по членству, обнажившийся после ADR-064 (один pocket-набор всех субъектов в БД). Амендирует ADR-039 §SQL-level feed cutoff (gektar-monitor-u7p)
- [[decisions/ADR-066-sse-membership-filter|ADR-066]] — SSE-лента (live + backfill) применяет membership-фильтр на том же per-connection predicate-слое, что и view-фильтр (ADR-052), достигая паритета с page-load (ADR-065). Новый accessor `RegionSubscriptionRepository.list_subscribed_region_ids() -> frozenset[int]` (snapshot на connect); новая фабрика `make_sse_membership_filter` композится с `make_sse_view_filter` через `and`. Один фикс-поинт в `GET /events` покрывает live **и** backfill (оба идут через `SseStreamer.stream(event_filter=...)`); `BrowserSseNotifier`/`BackfillService`/`LotQueryService` не тронуты. Инварианты V1–V5-SSE (пустое множество подписок → подавляет region-bearing, пропускает `region_id None` — паритет с page-load). Разблокирует счётчик 6jg. Расширяет ADR-052, supersedes ADR-039 §BrowserSseNotifier для membership (gektar-monitor-i7n)
- [[decisions/ADR-069-redirect-response-session-expired-guard|ADR-069]] — 3xx-redirect guard в `PaginatedListFetcher.iterate()` и `MonitorCycleService._run_cycle_inner()`: `300 <= status < 400 → raise SessionExpiredError("redirect status=N")` до вызова `parse()`. Устраняет ложный `ParseBugError` при HTTP 302 (пустое тело → missing tbody) — подтверждено прод-диагностикой 2026-06-05. `final_url` логируется только в `extra`, не в сообщении исключения (PII). `passes_subscription_cutoff` и `SelectolaxListParser` не изменены (bd gektar-monitor-31y)
