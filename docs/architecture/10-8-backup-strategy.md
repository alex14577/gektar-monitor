...
> **Альтернатива** (рассмотренная и отвергнутая): `VACUUM INTO 'backup.db'` всего `state.db`. Проще, но (а) бэкап раздувается до десятков МБ через год; (б) при восстановлении из бэкапа на новой машине mirror «застывает» — лучше пусть новый пользователь разберёт сайт заново.
