...
Сортировка лотов в фиде захардкожена: `ORDER BY date_create DESC, id DESC` (новее первым). UI-контрол выбора сортировки удалён — старые лоты менее интересны и не требуют ручного переключения. Cookie `view_filters` от старых версий, содержащая поле `sort_dir`, корректно игнорируется (silent ignore через Pydantic `extra="ignore"`).
