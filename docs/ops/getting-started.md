...
- **ОС для приёмочных тестов пользовательского сценария**: Windows 10/11 (VM подойдёт). На Windows проверяем single-instance lock, путь `%LOCALAPPDATA%`, поведение Notification API.
...
Сессия живёт в `profile/` (persistent context Playwright). Логин делает **сам пользователь** руками в открытом окне — приложение никогда не видит и не хранит пароль ЕСИА.
...
2. Открывается `НаДальнийВосток.рф` → пользователь жмёт «Войти через Госуслуги».
3. Пользователь проходит ЕСИА (логин, пароль, 2FA — всё в окне браузера).
