...
**Действие.**
1. В UI пользователь нажимает **«Перелогиниться»** (см. [[web/ui-architecture]]).
2. Сервер запускает Playwright в **headed** режиме с `user_data_dir=./profile`.
3. Пользователь проходит ЕСИА (логин, пароль, 2FA) в открывшемся окне.
...
**Действие.**
1. В UI → «Настройки → Email → SMTP override» — пользователь вводит **свой** SMTP (host/port/login/password). Логин/пароль сохраняются в `state.db` (таблица `smtp_credentials`), не в `config.json`.
2. Сохранить — изменения применяются к следующему циклу.
3. Повторить отправку через `POST /api/notifications/retry`.

**Предотвращение.** Мониторинг `notifier.email.auth_error` в логах. Документировать у пользователя: «если бот-ящик упал — переключаемся на свой SMTP, инструкция в UI». См. [[notifications]].
...
- В нормальном Chrome тот же пользователь логинится без проблем.
...
3. Попробовать залогиниться в обычном Chrome тем же пользователем — если работает, проблема в CfT.
...
**Предотвращение.** При первой установке тестировать ЕСИА-логин в headed-режиме. Если 1.58 ломается у пользователя — фиксируем 1.56 в его сборке.
