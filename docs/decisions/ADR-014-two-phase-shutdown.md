...
**Расширение (uvicorn graceful-shutdown deadline, bd gektar-monitor-3l8).** `uvicorn.Config` создаётся с `timeout_graceful_shutdown=5`. Без него (дефолт `None`) `Server.shutdown()` ждёт закрытия всех соединений бесконечно: открытый SSE-стрим `/events` (`StreamingResponse` поверх бесконечного async-генератора `SseStreamer.stream()`) держит коннект, а HTMX-SSE авто-реконнект не даёт счётчику соединений дойти до нуля — процесс не завершается даже после нескольких Ctrl+C, т.к. `lifespan.shutdown()` (раздача `stop_event` фоновым тредам) вызывается уже ПОСЛЕ ожидания соединений. С `timeout_graceful_shutdown=5` uvicorn через 5с принудительно отменяет ASGI-задачи (CancelledError разворачивает SSE-генераторы → `subscription.unsubscribe()`), затем выполняется `lifespan.shutdown()`. Значение 5с < grace 35с — uvicorn-drain идёт ПЕРЕД phase 1 (бюджеты складываются последовательно, не параллельно). Известный остаточный лаг: `sse_executor` worker-треды (`ThreadPoolExecutor`, non-daemon) могут жить до `_DEFAULT_PING_INTERVAL=15s` после shutdown, блокируясь в `queue.get(timeout=15)` — задерживает физический exit интерпретатора (не данные, не lock; subscription уже отписана). Ускорение (снижение ping-interval / проброс `stop_event` в `wait_one`) — отдельный follow-up.

**Расширение R3-M3 (Known limitations).**
- **Windows shutdown машины**: `WaitToKillAppTimeout` по умолчанию 5с — phase 1 grace=35с не успевает; in-flight notifications/lots могут не записаться. Документировано в [[ops/runbook]]: «при shutdown машины монитор не гарантирует доставку in-flight уведомлений». Будущее улучшение (не MVP): `SetConsoleCtrlHandler(CTRL_SHUTDOWN_EVENT)` fast-path с grace_timeout=4с.
- **systemd**: для корректного shutdown unit-файл должен иметь `TimeoutStopSec=52s` (uvicorn-drain 5с + grace 35с + phase 1.5 ≤5с + 7 запас). Указано в installer-скрипте и runbook.
