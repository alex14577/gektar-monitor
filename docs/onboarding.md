...
В первой версии middleware `onboarding_gate` редиректил на `?step=N` из query-param. Пользователь мог `GET /?step=4` и пропустить настройку SMTP. Это **дыра в guard-rails**, а не косметика — лот «по умолчанию» уйдёт через дефолтный бот-ящик без явного согласия пользователя.
