...
LATEST_SCHEMA_VERSION: Final[int] = 9
...
def init_db(
    ...
    *,
    schema_sql: str,
    latest_version: int = LATEST_SCHEMA_VERSION,
    ...
    Args:
        ...
        latest_version:   Expected ``PRAGMA user_version`` after initialisation.
                          Defaults to ``LATEST_SCHEMA_VERSION`` (matches ``docs/db/schema.sql``).
    ...
    if current > latest_version:
        raise RuntimeError(f"DB schema newer than app (got {current}, expected {latest_version})")
    ...
    if current == 0:
        ...
        if not has_tables:
            ...
            if actual != latest_version:
                raise RuntimeError(
                    f"Schema applied but user_version is {actual}, expected {latest_version}"
                )
            return
